...
interface BB {
	...
	abstract void test3();
	abstract void test4();
	void test5(); // 인터페이스에서는 abstract키워드를 생략 할 수 있다.
...
interface CC {
	...
	// 인터페이스에서는 abstract키워드를 생략 할 수 있다.
	void test8();
	void test9();
	void test10();
	
}
// implements키워드를 사용하여 인터페이스를 '구현' 한다.(상속x) 
class DD implements BB , CC {	// 인터페이스는 ,를 이용하여 다중구현이 가능하다. 
